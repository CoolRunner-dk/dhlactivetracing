...
class AccountNumberDataType extends AbstractStructBase
{
    ...
    public function setAccountType(array $accountType = array())
    {
        ...
        foreach ($accountType as $accountNumberDataTypeAccountTypeItem) {
            if (!\EnumType\AccountType::valueIsValid($accountNumberDataTypeAccountTypeItem)) {
                $invalidValues[] = var_export($accountNumberDataTypeAccountTypeItem, true);
            }
        }
        if (!empty($invalidValues)) {
            throw new \InvalidArgumentException(sprintf('Value(s) "%s" is/are invalid, please use one of: %s', implode(', ', $invalidValues), implode(', ', \EnumType\AccountType::getValidValues())), __LINE__);
        }
    ...
    public function addToAccountType($item)
    {
        ...
        if (!\EnumType\AccountType::valueIsValid($item)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $item, implode(', ', \EnumType\AccountType::getValidValues())), __LINE__);
        }
