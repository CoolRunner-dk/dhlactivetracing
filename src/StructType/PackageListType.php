...
class PackageListType extends AbstractStructBase
{
    ...
    public function setPackageData(array $packageData = array())
    {
        foreach ($packageData as $packageListTypePackageDataItem) {
            ...
            if (!$packageListTypePackageDataItem instanceof \StructType\PackageDataType) {
                throw new \InvalidArgumentException(sprintf('The packageData property can only contain items of \StructType\PackageDataType, "%s" given', is_object($packageListTypePackageDataItem) ? get_class($packageListTypePackageDataItem) : gettype($packageListTypePackageDataItem)), __LINE__);
            }
        }
    ...
    public function addToPackageData(\StructType\PackageDataType $item)
    {
        ...
        if (!$item instanceof \StructType\PackageDataType) {
            throw new \InvalidArgumentException(sprintf('The packageData property can only contain items of \StructType\PackageDataType, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
